...
const cashFlow = [
  { id: 1, date: "1404/08/20", description: "پرداخت فاکتور #123", type: "expense", amount: "1,250,000" },
  { id: 2, date: "1404/08/19", description: "دریافت از مشتری A", type: "income", amount: "2,800,000" },
  { id: 3, date: "1404/08/18", description: "خرید تجهیزات", type: "expense", amount: "750,000" },
  { id: 4, date: "1404/08/17", description: "فروش محصول B", type: "income", amount: "1,950,000" },
];
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-black/40 backdrop-blur-xl border-b border-white/10 px-4 py-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3 flex-1 max-w-md mx-4">
              ...
              <input type="text" placeholder="جستجو در مالی..." className="w-full bg-white/10 backdrop-blur-md rounded-xl pl-11 pr-4 py-2.5 text-white placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-yellow-500/50 transition-all" />
            </div>
        ...
        <main className="flex-1 p-4 md:p-6 space-y-6 overflow-y-auto">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-white drop-shadow-lg">مدیریت مالی</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { title: "درآمد کل", value: "۳۴,۵۶۷,۰۰۰ تومان", change: "+۱۸.۲%" },
              { title: "هزینه کل", value: "۲۲,۳۴۵,۰۰۰ تومان", change: "-۵.۱%" },
              { title: "سود خالص", value: "۱۲,۲۲۲,۰۰۰ تومان", change: "+۴۲.۳%" },
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white/10 backdrop-blur-2xl rounded-2xl p-5 border border-white/10">
              <h3 className="text-lg font-semibold text-white mb-4">درآمد و هزینه ماهانه</h3>
              <div className="h-64 sm:h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={monthlyRevenue}>
                    ...
                    <Legend />
                    <Line type="monotone" dataKey="revenue" stroke="#10b981" strokeWidth={3} name="درآمد" />
                    <Line type="monotone" dataKey="expense" stroke="#ef4444" strokeWidth={3} name="هزینه" />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="bg-white/10 backdrop-blur-2xl rounded-2xl p-5 border border-white/10 overflow-hidden">
              <h3 className="text-lg font-semibold text-white mb-4">جریان نقدی اخیر</h3>
                    ...
                    {cashFlow.map((row) => (
                      <tr key={row.id} className="border-b border-white/05 hover:bg-white/5 transition-all">
                        ...
                        <td className="py-3 px-4">
                          <span className={`px-2 py-1 rounded-full text-xs ${row.type === "income" ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}>
                            {row.type === "income" ? "درآمد" : "هزینه"}
                          </span>
                        </td>
                        <td className="py-3 px-4 font-mono">{row.amount} تومان</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
